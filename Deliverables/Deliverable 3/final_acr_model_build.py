# -*- coding: utf-8 -*-
"""Final ACR Model Build.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1fKgCq3YGGVxt2cbU77jG0AeKgAT0YcPq
"""
...
def create_model(input_shape, dropout_rate, l2_reg):
  """Building the CNN model
      :param input_shape (tuple): shape of input to CNN
      :param dropout_rate (float): droupout rate for neurons in each layer
      :param l2_reg (float): weight decay lambda values for each layer
      """
  ...
  # CNN layer 1
  model.add(keras.layers.Conv2D(32, (3,3), activation="relu", padding="same", input_shape=input_shape, kernel_regularizer=keras.regularizers.l2(l2_reg)))
  ...
  # CNN layer 3
  model.add(keras.layers.Conv2D(64, (3,3), activation="relu", padding="same", input_shape=input_shape, kernel_regularizer=keras.regularizers.l2(l2_reg)))
  model.add(keras.layers.MaxPool2D((3,3), strides=(2,2), padding="same"))
  model.add(keras.layers.BatchNormalization())
  model.add(keras.layers.Dropout(dropout_rate))
  ...
  model.add(keras.layers.Dense(128, activation="relu", kernel_regularizer=keras.regularizers.l2(l2_reg)))
  model.add(keras.layers.BatchNormalization())
  model.add(keras.layers.Dropout(dropout_rate))
...
def train_with(input_shape, dropout_rate, l2_reg, learning_rate):
  """Trains and evaluates the model taking in the argument of the function as hyperperameters.
      :param input_shape (tuple): shape of input to CNN.
      :param dropout_rate (float): droupout rate for neurons in each layer.
      :param l2_reg (float): weight decay lambda values for each layer.
      :param learning_rate (float): the learning rate used for the Adam optimizer.
      """

  # build the CNN model using the given hyperparameters
  model = create_model(input_shape, dropout_rate, l2_reg)
...
def optimize_model(partial_func, hp_bounds):
  """Optimizes the model by varying the hyperperamteres. Based on bayesian optimization.
      :param partial_func ("partial" object): the partial function which holds the input shape but will take in the arguments given in hp_bounds.
      :param hp_bounds (dict): dictionary containing the hyperperameter space that will be searched during optimization
      """ 

  # setting up dynamically changing bounds for the hyperperameter to improve search speed
  bounds_transformer = SequentialDomainReductionTransformer()
  
  # optimizing the hyperperameters
  optimizer = BayesianOptimization(partial_func, hp_bounds, bounds_transformer=bounds_transformer)
...
def get_confusion_matrix(true_label, predicted_label, mapped_labels, save_path, dim=16):
  ...
  figure_path = os.path.join(save_path, "Final_Figures")
...
if __name__ == "__main__":
  ...
  # setting the bounds for the hyperperameters and building
  hp_bounds = {'dropout_rate': (0.1, 0.7), 'l2_reg': (1e-4, 1e-2), 'learning_rate': (1e-4, 1e-2)}
  
  # evaluating, and optimizing the CNN model
  train_with_partial = partial(train_with, input_shape)
  optimal_hyperperameters = optimize_model(train_with_partial, hp_bounds)

  # printing optimal hyperperameters
  print(optimal_hyperperameters.max)

  # fetching optimal hyperperameters and creating model with optimal hyperperameters
  DROPOUT_RATE = optimal_hyperperameters.max['params']['dropout_rate']
  L2_REG = optimal_hyperperameters.max['params']['l2_reg']
  LEARNING_RATE = optimal_hyperperameters.max['params']['learning_rate']
  
  model = create_model(input_shape, dropout_rate=DROPOUT_RATE, l2_reg=L2_REG)
  ...
  # saving model
  model.save(os.path.join(NON_GUITAR_DATA_PATH, "Final_Model"))

# loading model
Final_Model = keras.models.load_model("/content/drive/MyDrive/AutoChordRecognition Dataset/non_guitar/Final_Model")
